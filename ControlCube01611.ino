/* 
Control Cube v0.1.6.1.1 [untested]
*/
...
char menuString2[10];
char delayString[10];
unsigned int winScore = 600; // Default score, ~8 minutes. (Score/2) = total hold time seconds to win.
int startDelay = 0;
int delayTime;
int delayTime2;
...
/*~~~~~~~~~~~MENU 1~~~~~~~~~~~*/
	...
	while(blueButtonState == HIGH || redButtonState == HIGH)
	{
		blueButtonState = digitalRead(blueButPin);
		redButtonState = digitalRead(redButPin);
	}
...
/*~~~~~~~~~~~BLUE STATE CHANGE~~~~~~~~~~~*/
  ...
  if(holdCount == 0)
  {
	 if (blueButtonState == HIGH && isBlueCounting == false)
	{
		sendStringBlue("ctrL");
		isBlueCounting = true;
		isRedCounting = false;
...
/*~~~~~~~~~~~RED STATE CHANGE~~~~~~~~~~~*/
	...
	if(holdCount == 0)
	{
		if (redButtonState == HIGH && isRedCounting == false)
		{
			sendStringRed("ctrL");
			isRedCounting = true;
